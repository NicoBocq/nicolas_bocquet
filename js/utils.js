...
const DataLoader = {
    ...
    loadProfile(profileElement, roleElement, emailElement, websiteElement) {
        ...
        if (profileElement.firstname) profileElement.firstname.textContent = p.firstname;
        if (profileElement.lastname)  profileElement.lastname.textContent  = p.lastname;
        if (profileElement.name)      profileElement.name.textContent      = `${p.firstname} ${p.lastname}`;
    ...
    loadExperience(container) {
        ...
        this._data.resume.experience.forEach(job => {
            ...
            if (job.details?.length) detailsHtml += `<ul>${job.details.map(d => `<li>${d}</li>`).join('')}</ul>`;
            item.innerHTML = `
                <h3>${job.role}</h3>
